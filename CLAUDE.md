...
| scheduler | `internal/kernel/scheduler` | dual-resource admission (memory + AI window), never-partial, priority+FCFS, opportunistic backfill, clock-refilled window |
...
Stubs still `doc.go`-only: `kernel/{interpreter,sessions,triggers,launcher}`,
...
1. **scheduler** — finish with patience (max_wait) + escalate/overclock.
2. **sessions/registry** (`kernel/sessions`) — track live instances, the drain
   check the library's deregister consults.
3. **triggers** (`kernel/triggers`) — cron + bus-event rules, sequence_id
   propagation across event hops.
4. **launcher** (`kernel/launcher`) — Docker SDK: run a service image with its
   envelope, scratch space, `.env`, bus wiring (integration-tested).
5. **tool vocabulary** (`docs/vocabulary/ai.md`, `connectors.md`) — the gating
   design work; the cfg grammar is derived from it.
6. **contracts** (`contracts/{cfg,bus,sdk}`) then the **interpreter**.
